    ...
    def predictPOS(self, sent):
        ...
        for i in range(len(valMatrix)):
            if f"{self.POSlist[i]}|{sent[0]}" not in self.emissionDict:
                valMatrix[i] = np.float64(0.0000000001)
            else:
                valMatrix[i] = np.float64(self.emissionDict[f"{self.POSlist[i]}|{sent[0]}"] * self.frequencyPOSNormalized[self.POSlist[i]]) + np.float64(0.0000000001)
                
        k = valMatrix.index(max(valMatrix))
        posPredicted.append(self.POSlist[k])
        ...
        for i in range(1, len(sent)):
            ...
            for j in range(len(valMatrix)):
                if f"{self.POSlist[j]}|{w}" not in self.emissionDict or f"{posPredicted[-1]}|{self.POSlist[j]}" not in self.transitionDict:
                    valMatrix[j] = np.float64(0.0000000001)
                else:
                    valMatrix[j] = np.float64(self.emissionDict[f"{self.POSlist[j]}|{w}"] * self.transitionDict[f"{posPredicted[-1]}|{self.POSlist[j]}"]) + np.float64(0.0000000001)
